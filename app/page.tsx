...
export default function Home() {
  ...
  // Updated loadBooks function with user authentication
  const loadBooks = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('books')
        .select('*')
        .eq('user_id', user.id)  // Only get books for current user
  ...
  // Updated handleAddBook function with user authentication
